...
import { ArrowLeft, ExternalLink, Play, Puzzle } from "lucide-react";
import { REMEDIAL_ACTIVITIES, type RemedialActivity } from "@/lib/remedial";
import { Button } from "@/components/ui/Button";
import { SectionCard } from "@/components/ui/SectionCard";

export function Remedial() {
  const [playing, setPlaying] = useState<RemedialActivity | null>(null);
  ...
  return (
    <SectionCard title="Remedial">
      <ul className="grid gap-2 sm:grid-cols-2">
        {REMEDIAL_ACTIVITIES.map((activity) => (
          <li
            key={activity.id}
